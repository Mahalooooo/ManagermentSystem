...
public class PageGrid : MonoBehaviour
{
	...
	public void UpadateUIPageFromHead()
	{
		...
		if (maxPageIndex > uiPageArray.Length)
		{
			foreach (var item in uiPageArray)
			{
				...
				{
					item.transform.GetChild(1).GetComponent<TMP_Text>().text = maxPageIndex.ToString();
				}
				else
				{
					item.transform.GetChild(1).GetComponent<TMP_Text>().text = headPageIndex.ToString();
				}
		...
		else
		{
			...
			{
				uiPageArray[i].transform.GetChild(1).GetComponent<TMP_Text>().text = headPageIndex.ToString();
	...
	public void UpdateUIPageFromEnd()
	{
		...
		if (maxPageIndex > uiPageArray.Length)
		{
			...
			for (int i = uiPageArray.Length - 1; i > 0; i--)
			{
				if (i == 0)
				{
					uiPageArray[i].transform.GetChild(1).GetComponent<TMP_Text>().text = "1";
				}
				else if (i == 1)
				{
					uiPageArray[i].transform.GetChild(1).GetComponent<TMP_Text>().text = "...";
				}
				else
				{
					uiPageArray[i].transform.GetChild(1).GetComponent<TMP_Text>().text = count.ToString();
					count--;
				}
			}
		}
	...
	public void UpdateUIPageFromMiddle(string number)
	{
		...
		for (int i = 0; i < uiPageArray.Length; i++)
		{
			...
			{
				uiPageArray[i].transform.GetChild(1).GetComponent<TMP_Text>().text = maxPageIndex.ToString();
			}
			else
			{
				uiPageArray[i].transform.GetChild(1).GetComponent<TMP_Text>().text = count.ToString();
		...
		EventManager.Send(EventType.JumpPage, number);
	...
	public UIPage FindUIPageWithText(string TMP_Text)
	{
		foreach (var item in uiPageArray)
		{
			if (item.transform.GetChild(1).GetComponent<TMP_Text>().text == TMP_Text)
			{
				return item;
			}
		}
	...
	public void OnBtnRight(string selection)
	{
		...
		{
			//当前页面是最后一页或者是第一页
			if (int.Parse(uiPage.transform.GetChild(1).GetComponent<TMP_Text>().text) == maxPageIndex && selection == "1" || int.Parse(uiPage.transform.GetChild(1).GetComponent<TMP_Text>().text) == 1 && selection == "-1")
			{
				return;
			}
			else
			{
				//跳转页面
				JumpPage((int.Parse(uiPage.transform.GetChild(1).GetComponent<TMP_Text>().text) + int.Parse(selection)).ToString());
			}
		}
